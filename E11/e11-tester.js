  ...
  try {
    ...
    if (match && match.length > 1) {
      ...
      if (match[1].trim() === expectedContent) {
        console.log(
          '馃帀 Auto-tester passed: The "auto-tester" div content matches the expected output!'
        );
        console.log(
          "Your deployment is live, and it鈥檚 amazing to see how far you鈥檝e come! Keep pushing forward.\nWow, you鈥檙e building the future, one line of code at a time. Great job!"
        );
        process.exit(0);
      } else {
        console.log(
          '鉂� Auto-tester failed: The content of the "auto-tester" div does not match the expected output.'
        );
        console.log(`Received: ${match[1].trim()}`);
        console.log(`Expected: ${expectedContent}`);
        console.log(
          "Don鈥檛 be discouraged -- every bug is just another step on the path to mastery."
        );
      }
    } else {
      console.log(
        '鉂� Auto-tester failed: The "auto-tester" div was not found in the response.'
      );
      console.log(
    ...
    console.log("鉂� Auto-tester failed:", error.message);
  } finally {
    process.exit(1);
  }
}
